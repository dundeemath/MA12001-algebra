...
In @fig-parallelogram, $\V{AD} + \V{DC} = \V{AC}$, and using the fact that $\V{DC} = \V{AB}$ and $\V{AD} = \V{BC}$, we see that $\V{DC} + \V{AD} = \V{AB} + \V{BC} = \V{AC}$, so we have proven that $\V{AD} + \V{DC} = \V{DC} + \V{AB}$.
